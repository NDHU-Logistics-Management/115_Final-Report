...
route_type_hualien$coast  <- c(1129, 1132, 1136, 1140, 1145, 8101, 8102, 8105, 8119)  # 含跨縣市
route_type_hualien$valley <- c(1121, 1122, 1128, 1130, 1135, 1137, 1139, 1142, 1143, 8161, 8173)  # 含跨縣市
route_type_hualien$cross  <- c(1125, 1133, 1141, 8181)  # 含跨縣市
...
# 查上下車站名
head(data_968[, c("上車站牌名稱", "下車站牌名稱", 
                  "資料代表日期(yyyy-MM-dd)", "業者編號")], 20)
# 再執行 join 就不會報錯
data_taitung <- data1 %>%
  inner_join(route_map_taitung, by = "搭乘路線名稱")

data_hualien <- data1 %>%
  inner_join(route_map_hualien, by = "搭乘路線名稱")
...
df_mileage_taitung <- data_taitung %>%
  ...
  # join上車累積里程
  left_join(
    bus_mileage_cum %>% select(搭乘路線名稱, 搭乘公車路線方向, 站序資料, 累積里程) %>%
      rename(上車里程 = 累積里程),
    by = c("搭乘路線名稱"     = "搭乘路線名稱",
           "搭乘公車路線方向" = "搭乘公車路線方向",
           "上車計費站序資料" = "站序資料")
...
down_na_ttt <- df_mileage_taitung %>%
  filter(is.na(下車里程)) %>%
  select(搭乘路線名稱, 下車站牌代碼, 搭乘公車路線方向) %>%
  distinct() %>%
  rename(站牌代碼 = 下車站牌代碼)

na_check_ttt <- bind_rows(up_na_ttt, down_na_ttt) %>%
  distinct() %>%
  arrange(搭乘路線名稱, 搭乘公車路線方向)
print(na_check_ttt)   
...
# 台東 ✅
png(paste0(path, yr_label, "臺東縣公路客運月平均搭乘里程折線圖.png"),
    width = 15, height = 5, units = "in", res = 300, family = "kai")
intercity_bus_monthly_average_mileage_plot(
  df_mileage_taitung,
  paste0(yr_label, "臺東縣公路客運月平均搭乘里程折線圖"))
dev.off()

# 花蓮 ✅
png(paste0(path, yr_label, "花蓮縣公路客運月平均搭乘里程折線圖.png"),
    width = 15, height = 5, units = "in", res = 300, family = "kai")
intercity_bus_monthly_average_mileage_plot(
  df_mileage_hualien,
  paste0(yr_label, "花蓮縣公路客運月平均搭乘里程折線圖"))
dev.off()
