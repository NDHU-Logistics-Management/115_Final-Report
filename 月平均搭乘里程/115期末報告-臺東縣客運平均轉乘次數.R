...
data_transfer <- data_taitung_all %>%
  mutate(
    ...
    是否轉乘 = ifelse(
      轉乘代碼紀錄 %in% c(202, 203, 302, 303),  # ← 只有這4個算轉乘
      1, 0
    )
  ) %>%
  filter(
  ...
  select(卡號, 業者編號, 資料日期, 年月, 票種分類, 是否轉乘, 轉乘代碼紀錄)

# 確認轉乘筆數
sum(data_transfer$是否轉乘)
table(data_transfer$轉乘代碼紀錄[data_transfer$是否轉乘 == 1])
unique(data_taitung_all$業者編號)
...
# ── 月轉乘次數 ────────────────────────────────────────────────
month_data <- data_transfer %>%
  group_by(年月) %>%
  summarise(
    # 將「是否轉乘」加總，就是當月的總轉乘次數
    月轉乘次數      = sum(是否轉乘, na.rm = TRUE),
    
    # 同時符合「有轉乘」且「票種是TPASS」的數量
    TPASS月轉乘次數 = sum(是否轉乘 == 1 & 票種分類 == "TPASS", na.rm = TRUE),  
