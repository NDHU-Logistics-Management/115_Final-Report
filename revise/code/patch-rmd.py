...
不符預期就中止，避免靜默漏改。修補項目（裁示日期與理由見 review/裁示紀錄.md）：
  P1 路徑改相對路徑、輸出改到 revise/output
  P2 C4：sum(原始票證筆數) → .N（一列一人次）
  P3 A1/A2/A6 圖上標記：空心三角形、113/11 垂直虛線
  P4 花蓮市區第 3 章改用替代指標「單日平均使用次數」（B2）
  P5 四張平均每日／單日平均圖取消資料點數值標籤
  P6 主標題置左；第 3 章 12 張標題補「折線圖」
  P7 里程 4 張圖標題刪「月」字
  P8 A7：TPASS 重複列去除 remove_duplicate_rows()
